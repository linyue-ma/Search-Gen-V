...
def process_data_and_save_to_parquet(input_jsonl_path: str, output_parquet_path: str, max_nuggets: int = 10):
    ...
    with open(input_jsonl_path, encoding='utf-8') as f:
        for line in f:
            ...
            for i in range(0, len(nuggets_list), max_nuggets):
                ...
                ground_truth = format_labels(labels, fmt)
                
                extra_info = {
                    "nuggets_count": len(nugget_texts),
                    "prompt_format_hint": fmt
                }
                data.append({
                    "data_source": "nuggetverifier",
                    "messages": conversation,
                    "tools": [],
                    "extra_info": extra_info,
                    #"extra_body": {"engine_kwargs": {"enable_thinking": False}},
                    "reward_model": {
    ...
    print(f"\n成功将数据从 {input_jsonl_path} 转换为 {output_parquet_path}")
    print(f"实际生成数据: 总数{total_records}")
...
if __name__ == "__main__":
    ...
    # input_file = '/wuxi_gpfs/user/malinyue/data/release/trainset/final.dl2122_gemini_aug.jsonl'
    # output_file = '/wuxi_gpfs/user/malinyue/data/release/trainset/final.dl2122_gemini_aug_dapo.parquet'

    input_file = '/wuxi_gpfs/user/malinyue/data/release/trainset/final.dl23_gemini_val.jsonl'
    output_file = '/wuxi_gpfs/user/malinyue/data/release/trainset/final.dl23_gemini_val_dapo.parquet'
